...
function VoiceRoomCard({ room }) {
  ...
  }, [isLoggedIn, loading, room.id, inviteCode, navigate, showToast]);
    ...
    <>
      ...
      >
        ...
        <div className="relative h-[160px]">
          {!room.backdrop || imgError || !imgLoaded ? (
            <div className="w-full h-full bg-gradient-to-br from-[#1a1a2e] to-[#12121e] flex items-center justify-center animate-pulse">
              <span className="text-white/20 text-[18px]">🎙️</span>
            </div>
          ) : null}
          ...
          <div className="absolute top-3 left-3 bg-[#3b82f6] text-white text-[10px] px-2 py-[2px] rounded-full">
            🎙️ LIVE
          </div>
        ...
        <div className="px-4 pt-[14px] pb-4">
          ...
          <div className="text-[11px] text-white/40 mb-3">Speakers: {room.speakers}</div>
          ...
          <button
            className="w-full rounded-[9px] py-[10px] text-[13px] font-bold text-white"
            style={{
              background: ACCENT,
...
export default memo(VoiceRoomCard);
